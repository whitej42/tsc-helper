        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-5">
            {items.map((item) => (
                <Link
                    key={item.id}
                    to={item.to}
                    className={`group flex flex-col bg-white dark:bg-surface-dark-card rounded-card shadow-card hover:shadow-card-hover dark:shadow-dark-card dark:hover:shadow-dark-card-hover transition-all duration-200 hover:-translate-y-1 overflow-hidden border border-gray-200 dark:border-surface-dark-border ${item.soon ? 'opacity-60 cursor-default pointer-events-none' : ''}`}
                    style={{ borderLeftColor: item.accent, borderLeftWidth: '4px' }}
                >
                    {/* Image */}
                    <div className="relative overflow-hidden h-40 bg-gray-100 dark:bg-surface-dark-alt">
                        <img
                            src={item.imgSrc}
                            alt={item.title}
                        ...
                        />
                        {item.soon && (
                            <div className="absolute top-2 right-2 bg-gray-900/80 text-white text-[10px] font-bold tracking-rail uppercase rounded-rail px-2 py-0.5">
                                Coming Soon
                            </div>
                        )}
                    </div>
                    ...
                    <div className="px-4 py-3">
                        <h3 className="font-rail font-bold tracking-rail uppercase text-sm mb-1 text-rail-navy-900 dark:text-white">
                            {item.title}
                        </h3>
